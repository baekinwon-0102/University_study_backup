...
void MoveRightBeforeGripForSignature1(uint8_t targetBlockSigmap) {
  if (targetBlockSigmap == SIGNATURE_1_SIGMAP) {
    SetMobileGoalVelocityForSyncWrite(dxl, 0, 0, 0, 0);
    ChangeMobilebaseToPositionMode();
    DriveDistanceAndMmPerSecAndDirection(dxl, SIGNATURE_1_GRIP_RIGHT_OFFSET_MM, DRIVE_DIRECTION_RIGHT);
...
void setup() {
  ...
  for (int missionIdx = 0 ; missionIdx < MISSION_BLOCK_CNT ; missionIdx++) {
    ...
    if (haveFoundBlock) {
      MoveRightBeforeGripForSignature1(targetBlockSigmaps[missionIdx]);
