...
ManipulatorPose ReadManipulatorPresentPoseFromEEPROM(uint8_t id);
...
void PrintManipulatorPoseListFromEEPROM() {
  for (int i = 1 ; i <= MANIPULATOR_POSE_ID_MAX_CNT ; i++) {
      ManipulatorPose manipulatorPose = ReadManipulatorPresentPoseFromEEPROM(i);
...
ManipulatorPose RunManipulatorPoseWithPoseDataInEEPROM(uint8_t id, int32_t operatingTimeMillis, float motor1Angle) {
  ManipulatorPose manipulatorPose = ReadManipulatorPresentPoseFromEEPROM(id);
