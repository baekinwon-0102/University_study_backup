...
void loop() {
  ...
  while(dxl.syncWrite(&sw_infos) != true) {}
  delay(2000);
  SetMobileGoalVelocityForSyncWrite(GOAL_VELOCITY_VALUE, 0, 0, GOAL_VELOCITY_VALUE);
  while(dxl.syncWrite(&sw_infos) != true) {}
  delay(2000);
  SetMobileGoalVelocityForSyncWrite(-GOAL_VELOCITY_VALUE, 0, 0, -GOAL_VELOCITY_VALUE);
  while(dxl.syncWrite(&sw_infos) != true) {}
  delay(2000);
  SetMobileGoalVelocityForSyncWrite(0, GOAL_VELOCITY_VALUE, GOAL_VELOCITY_VALUE, 0);
  while(dxl.syncWrite(&sw_infos) != true) {}
  delay(2000);
  SetMobileGoalVelocityForSyncWrite(0, -GOAL_VELOCITY_VALUE, -GOAL_VELOCITY_VALUE, 0);
  while(dxl.syncWrite(&sw_infos) != true) {}
